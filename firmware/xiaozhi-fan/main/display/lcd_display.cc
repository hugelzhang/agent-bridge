...
SpiLcdDisplay::SpiLcdDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel,
    ...
    // PSRAM is shared with audio processing + frame buffers — keep cache modest
    size_t psram_size_mb = esp_psram_get_size() / 1024 / 1024;
    if (psram_size_mb >= 8) {
        lv_image_cache_resize(512 * 1024, true);  // 512KB (was 2MB, 缩减以释放PSRAM给音频处理)
        ESP_LOGI(TAG, "Use 512KB of PSRAM for image cache");
    } else if (psram_size_mb >= 2) {
        lv_image_cache_resize(256 * 1024, true);  // 256KB
        ESP_LOGI(TAG, "Use 256KB of PSRAM for image cache");
    }
#endif
