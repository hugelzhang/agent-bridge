...
void AudioService::EnableVoiceProcessing(bool enable) {
    ...
    if (enable) {
        ...
        // AFE处理器 Stop→Start 之间加短暂延时，确保DSP状态完全清理
        vTaskDelay(pdMS_TO_TICKS(10));
        ...
        // AFE处理器保持一次性初始化，Start/Stop循环由处理器内部管理
    }
}
...
void AudioService::WaitForPlaybackQueueEmpty() {
    ...
    bool drained = audio_queue_cv_.wait_for(lock, timeout, [this]() {
        return service_stopped_ || (audio_decode_queue_.empty() && audio_playback_queue_.empty());
    });
    if (!drained) {
        ESP_LOGW(TAG, "WaitForPlaybackQueueEmpty timed out (decode=%u playback=%u), forcing drain",
                 (unsigned)audio_decode_queue_.size(), (unsigned)audio_playback_queue_.size());
        audio_decode_queue_.clear();
        audio_playback_queue_.clear();
        audio_queue_cv_.notify_all();
    }
