 ...
 * Level2 中断优先级, 绝不抢占 I2S/WiFi (Level1)
 */
...
typedef struct { uint8_t cmd; uint8_t data; } cmd_item_t;
...
{
    if (!hw_ready || len > UPROTO_MAX_DATA) {
        ESP_LOGW(TAG, "TX fail: hw_ready=%d len=%d (max=%d)", hw_ready, len, UPROTO_MAX_DATA);
        return false;
    }
    ...
    if (n != 4 + len) {
        ESP_LOGE(TAG, "TX err: cmd=0x%02X want=%d got=%d", cmd, 4 + len, n);
    }
    return (n == 4 + len);
}
        ...
        if (rx_pos >= 3 + rx_dlen + 1) {
            if (b == UPROTO_EOF) {
                ...
                case CMD_STATUS_REPORT:
                    if (d[1] >= 4) {
                        ESP_LOGI(TAG, "STATUS: chg=%d light=%d angle=%d fan=%d",
                                 d[2], d[3], d[4], d[5]);
...
/* ---- 接收任务 (500ms timeout, 优先级 1 — 最小化 CPU 唤醒) ---- */

static void rx_task(void *arg)
{
    uint8_t b;
    while (1) {
        if (uart_read_bytes(UART_PORT, &b, 1, pdMS_TO_TICKS(500)) > 0) rx_byte(b);
    }
}
...
static void queue_cmd(uint8_t cmd, uint8_t data)
{
    send_frame(cmd, &data, 1);
}
