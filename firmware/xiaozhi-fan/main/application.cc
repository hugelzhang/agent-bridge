...
#include "assets.h"
#include "settings.h"
#include "uart_proto.h"  // AI Fan 风扇 UART 协议
...
void Application::Initialize() {
    ...
    // AI Fan: UART 初始化在音频就绪后 (LOWMED ISR, 安全)
    uart_proto_init();
    ESP_LOGI(TAG, "UART fan OK");
    // 诊断查询：确认下位机是否在线
    uart_proto_query_status();
...
void Application::HandleStateChangedEvent() {
    ...
    switch (new_state) {
        ...
        case kDeviceStateIdle:
            ...
            audio_service_.EnableWakeWordDetection(true);
            // AI 风扇: 首次进入 Idle 后自动唤醒
            {
                static bool first_idle = true;
                if (first_idle && protocol_) {
                    first_idle = false;
                    Schedule([this]() {
                        vTaskDelay(pdMS_TO_TICKS(3000));
                        if (GetDeviceState() == kDeviceStateIdle && !protocol_->IsAudioChannelOpened()) {
                            ESP_LOGI(TAG, "Auto-wake on boot");
                            SetDeviceState(kDeviceStateConnecting);
                            Schedule([this]() {
                                if (GetDeviceState() != kDeviceStateConnecting) return;
                                Board::GetInstance().SetPowerSaveLevel(PowerSaveLevel::PERFORMANCE);
                                if (!protocol_->OpenAudioChannel()) { SetDeviceState(kDeviceStateIdle); return; }
                                ESP_LOGI(TAG, "Auto-wake: audio channel opened");
